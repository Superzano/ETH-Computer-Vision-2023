...
def main():
  ...
  '''
  Set the pose of the first image (e_im1) to identity, to place it at the origin of the world coordinate system
  '''
  e_im1.SetPose(np.eye(3), np.zeros_like(possible_relative_poses[0][1]))
  for relpose in possible_relative_poses:
    '''
    Set the pose of the second image (e_im2) for the current relative pose
    '''
    e_im2.SetPose(relpose[0], relpose[1])
    '''
    Triangulate points between the first and the second image with their current poses
    '''
    points3D, im1_corrs, im2_corrs = TriangulatePoints(K, e_im1, e_im2, e_matches)
    '''
    Check if the current pose results in the maximum number of triangulated points
    Keep track of the pose that yields the most points in front of both cameras
    '''
  ...
  # The first camera pose is set to identity (meaning it's at the origin looking along the Z axis)
  '''
  After evaluating all possible poses, set the best found poses for both images
  These poses are assumed to be the transformation from the world coordinates to the image space.
  '''
